...
use App\Http\Requests\Api\Clients\RecordRequest;
...
class RecordController extends Controller
{
    ...
    protected $model;
    private $_action = [
        'index' => ['group_name' => 'record', 'action_name' => 'select'],
        'create' => ['group_name' => 'record', 'action_name' => 'add'],
        'update' => ['group_name' => 'record', 'action_name' => 'edit'],
        'delete' => ['group_name' => 'record', 'action_name' => 'delete'],
    ];
    private $not_permission_action_name = [];

    public function __construct(RecordRequest $request, Record $record, Actions $actions)
    {
    ...
    public function index(DiyCommonField $commonField, CommonFieldHandler $fieldHandler)
    {
        ...
        $data = $list->toArray();
        $diyField = $commonField->getCorporateDiyCommonFieldWith('record');

        $value = $fieldHandler->getValue(array_column($diyField->toArray(), 'id'), array_column($data['data'], 'id'));

        return $this->response->paginator($list, new RecordTransformer())
